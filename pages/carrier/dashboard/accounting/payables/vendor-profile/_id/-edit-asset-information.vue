...
export default Vue.extend({
  ...
  computed: {
    ...
    carrierAssets (): SelectOption[] {
      const assetOptions: SelectOption[] = []
      this.$store.getters['accounting/assetsDetails/asset'].forEach((asset: AssetsDetails) => {
        assetOptions.push({
          id: asset.id,
          text: `${asset.asset_type} ${asset.asset_amount}`,
          selected: false
        } as SelectOption)
      })
      return assetOptions
  ...
  created () {
    this.getAssetsByCarrierId({ page_number: 1, id: 1 }).finally(() => {
      this.getAsset(this.editId).then(() => {
        this.request = { ...this.asset }
      })
    })
  },
  methods: {
    ...mapActions({
      getAsset: (action, param) => action('accounting/assetsDetails/getAsset', param),
      updateAsset: (action, param) => action('accounting/assetsDetails/updateAsset', param),
      getAssetsByCarrierId: (action, param) => action('accounting/assetsDetails/getAssetsByCarrierId', param)
    }),
    handleSubmit () {
      this.updateAsset({
        ...this.request,
