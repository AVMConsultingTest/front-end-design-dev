<template>
  <div class="w-full">
    ...
    <div class="border border-gray-300/50 bg-gray-300/20 w-full rounded-lg flex flex-row mb-4 filter-tabs p-2">
      <div
        class="filter-tab"
        :class="[{ 'bg-white shadow-md shadow-black/10': vfilter.status === undefined }]"
        @click="changeFilter()"
      >
      ...
      <div
        class="filter-tab"
        :class="[{ 'bg-white shadow-md shadow-black/10': vfilter.status === 'UPCOMING' }]"
      ...
      <div
        class="filter-tab"
        :class="[{ 'bg-white shadow-md shadow-black/10': vfilter.status === 'IN-TRANSIT' }]"
      ...
      <div
        class="filter-tab"
        :class="[{ 'bg-white shadow-md shadow-black/10': vfilter.status === 'COMPLETED' }]"
    ...
    <div class="block my-8">
      <span class="text-b-3">{{ loads.length ?? 0 }} result found for <b class="capitalize">"{{ `${vfilter.status?.toLowerCase() ?? 'All'} ` }}Loads"</b></span>
    </div>
          ...
          >
            ...
            <template #item-load_complete_date="{ item }">
              <div class="block whitespace-nowrap">
                {{ formatDate(item.load_complete_date) }}
              </div>
            </template>
            ...
            <template #item-primary_driver-name="{ item }">
              <div class="block text-dark-blue whitespace-nowrap">
                {{ item.primary_driver.name }}
              </div>
            </template>
            <template #item-secondary_driver-name="{ item }">
              <div class="block text-dark-blue whitespace-nowrap">
                {{ item.secondary_driver.name }}
              </div>
            </template>
            <template #item-truck-status="{ item }">
              <div class="block text-dark-blue whitespace-nowrap">
                {{ item.truck.status }} 
              </div>
            </template>
            <template #item-trailer-status="{ item }">
              <div class="block text-dark-blue whitespace-nowrap">
                {{ item.trailer.status }} 
              </div>
            </template>
