...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      testOptionsDocumentCategory: [
        ...
        { id: 9, text: 'TEST DOCUMENT CATEGORY', selected: false }
      ] as SelectOption[],
  ...
  created () {
    this.getDriversByCarrierId({ page_number: 1, id: 1 }).finally(() => {
      this.getDocument(this.editId).then(() => {
        this.request = { ...this.document, carrier_id: this.document.carrier_id.toString() }
      })
    })
  },
  methods: {
    ...mapActions({
      getDocument: (action, id) => action('documents/getDocument', id),
      updateDocument: (action, params: Document) => action('documents/updateDocument', params),
      getDriversByCarrierId: (action, param) => action('driver/getDriversByCarrierId', param)
    }),
    handleSubmit () :void {
      const rest = {
        carrier_id: this.request.carrier_id.toString(),
        document_upload_date: moment(this.request.document_upload_date).toISOString(),
        driver_id: this.request.driver_id.toString()
        // document_category: '',
      ...
      this.updateDocument({ ...this.request, ...rest }).then(() => {
