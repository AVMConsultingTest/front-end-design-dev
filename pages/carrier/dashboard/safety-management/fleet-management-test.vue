  ...
  <div class="w-full">
    ...
    <div class="row">
      ...
      <div class="col-12 mb-6">
        ...
        >
          <template #item-truck_status="{ item }">
            <Status
              :item="item"
          ...
          <template #item-permit_delivery_status="{ item }">
            <Status
              :item="item"
              :active="extendeds.includes(item.id)"
              :item-status="item.permit_delivery_status"
                ...
                <div v-if="opened" class="z-[8] absolute top-10 rounded-lg bg-white right-0 w-48 shadow-2xl">
                  <ul class="px-4 py-5">
                    <li
                      ...
                      :class="[
                        { 'mb-7' : index !== decideAddNew.length - 1}
                      ]"
                      @click="action.click"
                    >
                      {{ action.name }}
...
export default {
  ...
  data () {
    ...
    return {
      ...
      dropdownStyle: 'a',
      dropdownMenuTruck: [
        { text: 'Moving', bg: 'green' },
        { text: 'In Service', bg: 'red' },
        { text: 'Parked', bg: 'orange' }
      ],
      dropdownMenuPermit: [
  ...
  methods: {
    ...
    setStatusTruck (id:number, newStatus:any) {
      ...
      this.updateTruck({
        ...
        out_of_service_date: moment(item.out_of_service_date).toISOString(),
        registration_renewal_date: moment(item.registration_renewal_date).toISOString(),
        annual_inspection_date: moment(item.annual_inspection_date).toISOString(),
        in_service_date: moment(item.in_service_date).toISOString(),
        preventive_maintenance_date: moment(item.preventive_maintenance_date).toISOString()
      })
    },
    ...
    setStatusPermit (id:number, newStatus:any) {
      ...
      this.updatePermit({
        ...item,
        permit_delivery_status: newStatus,
        carrier_id: item.carrier_id.toString(),
        truck_id: item.truck_id.toString(),
        permit_purchase_date: moment(item.permit_purchase_date).toISOString(),
        permit_end_date: moment(item.permit_end_date).toISOString(),
        permit_start_date: moment(item.permit_start_date).toISOString()
      })
    },
