...
export default Vue.extend({
  ...
  mounted () {
    this.getAlert(this.editId).then(() => {
      setTimeout(() => {
        console.log(this.alert)
        this.request = { ...this.alert }
      }, 1000)
    })
  },
  methods: {
    ...mapActions({
      getAlert: (action, id) => action('alerts/getAlert', id),
      updateAlert: (action, param) => action('alerts/updateAlert', param),
