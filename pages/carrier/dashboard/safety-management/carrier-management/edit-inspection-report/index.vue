...
export default Vue.extend({
  ...
  computed: {
    report (): InspectionsDetails {
      return this.$store.getters['inspectionReport/inspection']
    }
  },
  mounted () {
    this.getInspection(this.editId).then(() => {
      setTimeout(() => {
        ...
        this.request = { ...this.report }
      }, 1000)
    })
  },
  methods: {
    ...mapActions({
      getInspection: (action, id) => action('inspectionReport/getReport', id),
      updateInspection: (action, params) => action('inspectionReport/updateReport', params)
    }),
    handleSubmit (): void {
