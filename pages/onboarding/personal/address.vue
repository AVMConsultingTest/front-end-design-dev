<template>
  ...
  >
    <div class="flex flex-col self-end items-baseline row-start-1 row-end-1">
      <Badge
        title="Step: 2/2"
        class="bg-green-200 text-green-800 mb-2"
    ...
    <Form fclass="row my-8" class="row-start-2 row-span-2" @send="handleSubmit">
      <div class="col-6 mb-6">
        <Input
          v-model="request.user_address_line1"
          name="address1"
          type="text"
      ...
      <div class="col-6 mb-6">
        <Input
          v-model="request.user_address_line2"
          name="address2"
          type="text"
      ...
      <StateSelects :city.sync="request.user_city" :state.sync="request.user_state" />
      <div class="col-6 mb-6">
        <Input
          v-model="request.user_zip_code"
          name="zip"
          type="text"
          label="Zip Code *"
          rules="required"
...
export default Vue.extend({
  name: 'OnboardingPersonalAddress',
  ...
  watch: {
    'request.contact_state' (oldVal, newVal) {
      if (oldVal !== newVal) {
        this.getCities(this.request?.contact_state)
