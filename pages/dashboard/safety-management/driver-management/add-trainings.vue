<template>
  <keep-alive>
    <nuxt-child @close-drawer="$emit('close-drawer')" />
  </keep-alive>
</template>
...
export default Vue.extend({
  ...
  computed: {
    step () {
      return (this.$store.getters['driver/step'])
    }
  },
  mounted () {
    this.$router.push('/carrier/dashboard/safety-management/driver-management/add-trainings')
  },
  methods: {
    handleSubmit () {
      console.log('in')
