  ...
  <div class="h-full">
    <div class="pl-24 pr28">
      ...
      <Form fclass="row my-2" @send="handleSubmit">
        <div class="flex-col justify-center items-start mb-6 flex" style="flex: 0.5">
        ...
        <div class="col-12 mb-10">
          <Input
            class="font-medium pb-6 mb-1 mt-4 w-full border-b"
            :minimal="true"
          />
        </div>
...
export default Vue.extend({
  ...
  computed: {
    ...
    carrierTasks (): SelectOption[] {
      const taskOptions: SelectOption[] = []
      this.$store.getters['accounting/taskDetails/task'].forEach((task: TaskDetails) => {
        taskOptions.push({
          id: task.id,
          text: `${task.assigned_to} ${task.email_address}`,
          selected: false
        } as SelectOption)
      })
      return taskOptions
  ...
  created () {
    this.getTasksByCarrierId({ page_number: 1, id: 1 }).finally(() => {
      this.getTask(this.editId).then(() => {
  ...
  methods: {
    ...mapActions({
      ...
      updateTask: (action, param) => action('accounting/taskDetails/updateTask', param),
      getTasksByCarrierId: (action, param) => action('accounting/taskDetails/getTasksByCarrierId', param)
    }),
    handleSubmit () {
      this.updateTask(this.request).then(() => {
